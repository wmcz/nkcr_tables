...
from wikibaseintegrator.wbi_enums import WikibaseRank, ActionIfExists
...
select ?item ?label ?name1 ?name2 where {
  
   ?item wdt:P691 [] .
   ?item rdfs:label ?label filter(lang(?label)="cs" && regex(?label,"^[^ ]+ [^ ]+$") ) .
   BIND(REPLACE(STR(?label), "^[^ ]+ ", "") AS ?name1) 
   BIND(REPLACE(STR(?label), " [^ ]+$", "") AS ?name2)
   FILTER(?name1 = ?name2)
} 
        """
